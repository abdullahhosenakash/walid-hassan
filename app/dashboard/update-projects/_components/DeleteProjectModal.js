...
import { deleteProject } from '@/app/_lib/deleteFunctions/deleteProject';
      ...
      >
        <div>
          <p>
            ...
            {selectedProjectToDelete?.projectsLength && ' set'}?
          </p>
          <p className='text-slate-700 text-base mt-2 flex gap-2'>
            <span>Project Type: {selectedProjectToDelete?.projectType}</span>
            <input
              type='text'
              name='projectType'
              className='hidden'
              defaultValue={selectedProjectToDelete?.projectType}
            />
          </p>
          {selectedProjectToDelete?.projectsLength ? (
            <p className='text-slate-700 text-base'>
              This project set contains{' '}
              {selectedProjectToDelete?.projectsLength} project
              {selectedProjectToDelete?.projectsLength > 1 && 's'}
            </p>
          ) : (
            <>
              <p className='text-slate-700 text-base flex gap-2'>
                <span>
                  Project Name: {selectedProjectToDelete?.projectName}
                </span>
                <input
                  type='text'
                  name='projectId'
                  className='hidden'
                  defaultValue={selectedProjectToDelete?.projectId}
                />
              </p>
            </>
          )}
        ...
        <div>
          ...
          <div className='flex gap-4 justify-end'>
            <button
              ...
              onClick={() => {
                setSelectedProjectToDelete({});
                setDeleteSelectedProject(false);
              }}
            >
              Cancel
            </button>
