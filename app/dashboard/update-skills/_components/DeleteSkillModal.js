...
const DeleteSkillModal = ({
  setDeleteSelectedSkill,
  selectedSkill,
  setSelectedSkill
}) => {
  const [errorMessage, setErrorMessage] = useState(INITIAL_STATE);
  const [state, formAction] = useFormState(deleteSkill, INITIAL_STATE);
      ...
      >
        <div>
          ...
          <p className='text-slate-700 text-base mt-2 flex gap-2'>
            <span>Skill Type: {selectedSkill?.skillType}</span>
            <input
              type='text'
              name='skillType'
              className='hidden'
          ...
          {selectedSkill?.skillsLength ? (
            <p className='text-slate-700 text-base'>
              This skill set contains {selectedSkill?.skillsLength} skill
              {selectedSkill?.skillsLength > 1 && 's'}
            </p>
          ) : (
            <>
              <p className='text-slate-700 text-base flex gap-2'>
                <span>Skill Name: {selectedSkill?.skillName}</span>
                <input
                  type='text'
                  name='skillName'
                  className='hidden'
              ...
              <p className='text-slate-700 text-base flex gap-2'>
                <span> Percentage: {selectedSkill?.percentage}</span>
                <input
                  type='text'
                  name='percentage'
                  className='hidden'
