...
const DeleteResearchPaperModal = ({
  setDeleteSelectedResearchPaper,
  selectedResearchPaperToDelete,
  ...
  useEffect(() => {
    ...
    if (state?.status === 'success') {
      setDeleteSelectedResearchPaper(false);
      toast.success('Research paper(s) deleted successfully!');
      push('/research-papers');
    }
  }, [state, push, setDeleteSelectedResearchPaper]);
      ...
      >
        <div>
          <p>
            ...
            {selectedResearchPaperToDelete?.papersLength && ' set'}?
          </p>
          <p className='text-slate-700 text-base mt-2 flex gap-2'>
            <span>Paper Type: {selectedResearchPaperToDelete?.paperType}</span>
            <input
              type='text'
              name='paperType'
              className='hidden'
              defaultValue={selectedResearchPaperToDelete?.paperType}
            />
          </p>
          {selectedResearchPaperToDelete?.papersLength ? (
            <p className='text-slate-700 text-base'>
              This research paper set contains{' '}
              {selectedResearchPaperToDelete?.papersLength} paper
              {selectedResearchPaperToDelete?.papersLength > 1 && 's'}
            </p>
          ) : (
            <>
              <p className='text-slate-700 text-base flex gap-2'>
                <span>
                  Paper Name: {selectedResearchPaperToDelete?.paperName}
                </span>
                <input
                  type='text'
                  name='paperId'
                  className='hidden'
                  defaultValue={selectedResearchPaperToDelete?.paperId}
                />
              </p>
            </>
          )}
...
export default DeleteResearchPaperModal;
